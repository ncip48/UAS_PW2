...
<th>Penilaian</th>
			...
			for ($i = 0; $i < sizeof($minggu); $i++) {
				...
				#looping for kemampuan_arr and display in td when kemampuan_akhir is not empty
				// foreach ($kemampuan_arr as $key => $value) {
				// 	echo "<td>" . $value['kemampuan_akhir'] . "</td>";
				// }

				$kemampuanName = $kemampuan_arr[$i];

				if ($kemampuanName['kemampuan_akhir'] != '') {
					echo "<td>" . $kemampuanName['kemampuan_akhir'] . "</td>";
				} else {
					if ($kemampuanName['remove_bottom_border'] == true) {
						echo "<td style='border-bottom:1px solid transparent'></td>";
					}
				}

				if ($indikator_arr[$indikatorName]['printed'] == 'no') {
					echo "<td>" . $indikatorName . "</td>";
					$indikator_arr[$indikatorName]['printed'] = 'yes';
				} else {
					echo "<td></td>";
				}
				if ($topik_arr[$topikName]['printed'] == 'no') {
					echo "<td>" . $topikName . "</td>";
					$topik_arr[$topikName]['printed'] = 'yes';
				} else {
					echo "<td></td>";
				}
				if ($aktivitas_arr[$aktivitasName]['printed'] == 'no') {
					echo "<td>" . $aktivitasName . "</td>";
